...
public class Bluetooth {
        ...
        public void onReceive(Context context, Intent intent) {
          ...
          if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
            ...
            if (state == BluetoothDevice.BOND_BONDED && prevState == BluetoothDevice.BOND_BONDING) {
              if (discoveryCallback != null) discoveryCallback.onPair(devicePair);
  ...
  private ReactContext activity;
  private BroadcastReceiver mReceiverScan =
      new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
          String action = intent.getAction();
          ...
          switch (action) {
            case BluetoothAdapter.ACTION_STATE_CHANGED:
              ...
              if (state == BluetoothAdapter.STATE_OFF) {
                if (discoveryCallback != null) discoveryCallback.onError("Bluetooth turned off");
              }
              break;
            case BluetoothAdapter.ACTION_DISCOVERY_FINISHED:
              context.unregisterReceiver(mReceiverScan);
              if (discoveryCallback != null) discoveryCallback.onFinish();
              break;
            case BluetoothDevice.ACTION_FOUND:
              BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
              if (discoveryCallback != null) discoveryCallback.onDevice(device);
              break;
          }
        }
      };

  public Bluetooth(ReactContext reactContext) {
  ...
  public void disconnect() {
    try {
      ...
      if (communicationCallback != null) communicationCallback.onError(e.getMessage());
    }
  }
  ...
  public void send(String msg) {
    try {
      ...
      connected = false;
      if (communicationCallback != null) communicationCallback.onDisconnect(device, e.getMessage());
    }
  }
  ...
  public void pair(BluetoothDevice device) {
    activity.registerReceiver(
        mPairReceiver, new IntentFilter(BluetoothDevice.ACTION_BOND_STATE_CHANGED));
    devicePair = device;
    try {
      Method method = device.getClass().getMethod("createBond", (Class[]) null);
      method.invoke(device, (Object[]) null);
    } catch (Exception e) {
      if (discoveryCallback != null) discoveryCallback.onError(e.getMessage());
    }
  }
    ...
    public void run() {
      ...
      try {
        while ((msg = input.readLine()) != null) {
          if (communicationCallback != null) communicationCallback.onMessage(msg);
        }
      } catch (IOException e) {
        connected = false;
  ...
  private class ConnectThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        if (communicationCallback != null) communicationCallback.onConnect(device);
      } catch (IOException e) {
        if (communicationCallback != null)
